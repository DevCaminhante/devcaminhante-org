  ...
  template: `
    <p-dataView
      ...
      <ng-template pTemplate="header">
        <h1 [ngClass]="{ 'text-center': true }">{{ HeaderRouteText.BLOG }}</h1>
      </ng-template>
      <ng-template
        let-post
        pTemplate="listItem">
        <div [ngClass]="{ 'col-12 py-2 flex flex-column text-center': true }">
          ...
          <p [ngClass]="{ 'mt-0 text-sm': true }"
            >{{ post.attributes.date }} por {{ post.attributes.author }}</p
          >
        </div>
      </ng-template>
    </p-dataView>
...
export default class BlogIndexPage {
  ...
  readonly posts: ContentFile<Post>[] = injectContentFiles<Post>((file) =>
    file.filename.includes(Constant.BLOG_POSTS_DIRECTORY)
  ).map((post) => {
